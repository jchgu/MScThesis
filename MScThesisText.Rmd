...
Three features make China a suitable case for studying left-behind children. The first feature is the huge scale of internal labor migration. According to the National Bureau of Statistics, the year 2021 recorded 292.5 million in-country migrant workers [@nbs2022]. They form the backbone of essential industries, such as retail, delivery, transportation, and manufacturing. The 2015 one-percent National Population Sample Survey estimated that 68.7 million Chinese children were left behind [@nbs2017]. The internal migration stemmed from the neoliberal economic regime.
...
```{r i-var, fig.cap='Parental migration status at follow-up survey'}
...
```{r fig.cap='Dependent variables by parental migration'}
tab_dpvar |> 
  kable(booktabs = T, position = "left", row.names = F) |>
  add_indent(c(2,3,5,6)) |>
  add_header_above(c(" ", "No = 3452" = 2, "Yes = 636" = 2)) |>
...
```{r fig.cap='Dichotomous control variables by parental migration'}
tab_ctvar_dit |>
  kable(booktabs = T, position = "left", row.names = F) |>
  add_header_above(c(" ", "No = 3452" = 2, "Yes = 636" = 2)) |>
  add_header_above(c(" ", "Parental Migration" = 4)) |>
...
```{r fig.cap='Continuous control variables by parental migration'}
tab_ctvar_cnt |>
  kable(booktabs = T, position = "left", row.names = F) |>
  add_header_above(c(" ", "No = 3452" = 2, "Yes = 636" = 2)) |>
  add_header_above(c(" ", "Parental Migration" = 4)) |>
...
```{r v-plot-ttl-1, fig.cap = paste("(ref:ttl-dif)", "(ref:migr-type)")}
