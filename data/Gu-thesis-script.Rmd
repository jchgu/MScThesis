---
title: "Effect of Parental Absence on Adolescents' Academic Performance"
...
```{r}
d <- d %>% mutate(
  age = 2013 - a02a, #children's age at baseline
...
```{r}
d_cl <- d_cl %>% #Total exam grades at wave 1 and at wave 2
  mutate(w1ttl = tr_chn + tr_mat + tr_eng, 
         w2ttl = w2chn + w2mat + w2eng) 
d_cl <- d_cl %>% #Within-school z-scores for Chinese, math, English exams and their total, respectively
  group_by(schids) %>%
